...
- 221 个元素已按整车、出餐窗口、菜单、车轮、HUD、轮播、导航和弹窗整理成中文折叠分类。
...
- 对齐计算会换算餐车镜头的 90% 缩放，轮子贴边不再留下约 5.8px 的视觉误差。
- 21 项自动化测试通过；真实页面已验证前轮左对齐、12px 边距吸附、水平居中、1px 微调和透视，控制台 0 错误。
...
网格、边距和自动吸附偏好保存在 `burger.home.editor-guides.v1`；这些同样只存在当前浏览器。跨电脑时最重要的 UI 结果仍以“下载调整文件”导出的 JSON 为准。
